...
function createClientApi(client) {
    return {
        ...
        /**
             ...
             * @description Liveness-style health check. Returns 200 as soon as the HTTP API is serving requests. It does not inspect collections, shards or consensus state, and is identical to `/livez`. Use it only to detect whether the undefined is up and responsive.
             */
        healthz: client
            .path('/healthz')
            .method('get')
        ...
        /**
             ...
             * @description Kubernetes liveness probe. Returns 200 as soon as the HTTP API is serving requests. It does not inspect collections, shards or consensus state, and is identical to `/healthz`. A failure indicates the undefined is unresponsive and should be restarted.
             */
        livez: client
            .path('/livez')
            .method('get')
...
let _current = {};
function withHeaders(headers, fn) {
...
class QdrantClient {
    constructor({ url, host, apiKey, https, prefix, port = 6333, timeout = 300_000, checkCompatibility = true, ...args } = {}) {
     ...
     *         If timeout is reached, request will return with service error.
     */
    async createCollection(collection_name, { timeout, vectors, hnsw_config, on_disk_payload, optimizers_config, quantization_config, replication_factor, shard_number, sharding_method, wal_config, write_consistency_factor, sparse_vectors, strict_mode_config, payload, metadata, }) {
     ...
     *         If timeout is reached, request will return with service error.
     */
    async recreateCollection(collection_name, { timeout, vectors, hnsw_config, on_disk_payload, optimizers_config, quantization_config, replication_factor, shard_number, sharding_method, wal_config, write_consistency_factor, sparse_vectors, strict_mode_config, payload, metadata, }) {
     ...
     *     - max_resident_memory_percent: Reject memory-consuming updates once undefined resident memory reaches
     ...
     * @returns Operation result
     */
    async updateQuotas({ wait, ...config } = {}) {
...
export { QdrantClient, QdrantClientConfigError, QdrantClientResourceExhaustedError, QdrantClientTimeoutError, QdrantClientUnexpectedResponseError, withHeaders };
